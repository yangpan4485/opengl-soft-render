#include "opengl_render_texture_compress_sub.h"
...
OpenGLRenderTextureCompressSub::OpenGLRenderTextureCompressSub() {
}

OpenGLRenderTextureCompressSub::~OpenGLRenderTextureCompressSub() {
}

bool OpenGLRenderTextureCompressSub::Init() {
    ...
    // testimage_RGBA8.pkm
    // container.pkm
    std::string texture_path = process_path_ + "/resource/testimage_RGBA8.pkm";
    texture_ = LoadTexture(texture_path.c_str());
    return true;
}

void OpenGLRenderTextureCompressSub::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTextureCompressSub::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTextureCompressSub::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;
}

bool OpenGLRenderTextureCompressSub::DrawGraphics(uint32_t width, uint32_t height)  {
...
uint32_t OpenGLRenderTextureCompressSub::LoadTexture(const std::string& texture_path) {
    uint32_t texture_id;
    ...
    glCompressedTexImage2D(GL_TEXTURE_2D, 0, format, width * 2, height * 2, 0, size, NULL);
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, format, size, data.c_str() + 16);
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, width, 0, width, height, format, size, data.c_str() + 16);
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, height, width, height, format, size, data.c_str() + 16);
    glCompressedTexSubImage2D(GL_TEXTURE_2D, 0, width, height, width, height, format, size, data.c_str() + 16);
