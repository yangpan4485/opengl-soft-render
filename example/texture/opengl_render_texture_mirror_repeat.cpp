#include "opengl_render_texture_mirror_repeat.h"
...
OpenGLRenderTextureMirrorRepeat::OpenGLRenderTextureMirrorRepeat() = default;

OpenGLRenderTextureMirrorRepeat::~OpenGLRenderTextureMirrorRepeat() = default;

bool OpenGLRenderTextureMirrorRepeat::Init() {
    ...
    std::string texture_path = process_path_ + "/resource/awesomeface.png";
...
void OpenGLRenderTextureMirrorRepeat::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTextureMirrorRepeat::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTextureMirrorRepeat::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderTextureMirrorRepeat::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderTextureMirrorRepeat::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
