#include "opengl_render_texture_wrap_repeat.h"
...
OpenGLRenderTextureWrapRepeat::OpenGLRenderTextureWrapRepeat() = default;

OpenGLRenderTextureWrapRepeat::~OpenGLRenderTextureWrapRepeat() = default;

bool OpenGLRenderTextureWrapRepeat::Init() {
...
void OpenGLRenderTextureWrapRepeat::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTextureWrapRepeat::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTextureWrapRepeat::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderTextureWrapRepeat::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderTextureWrapRepeat::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
