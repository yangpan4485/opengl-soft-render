#include "opengl_render_read_pixel.h"
...
OpenGLRenderReadPixel::OpenGLRenderReadPixel() = default;

OpenGLRenderReadPixel::~OpenGLRenderReadPixel() = default;

bool OpenGLRenderReadPixel::Init() {
    glGenTextures(1, &pixel_texture_);
    glBindTexture(GL_TEXTURE_2D, pixel_texture_);
...
void OpenGLRenderReadPixel::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &pixel_texture_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderReadPixel::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderReadPixel::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderReadPixel::DrawGraphics(uint32_t width, uint32_t height) {
    ...
    glBindTexture(GL_TEXTURE_2D, pixel_texture_);
    uint8_t* data = new uint8_t[window_width_ * window_height_ * 4];
    glReadPixels(0, 0, window_width_, window_height_, GL_RGBA, GL_UNSIGNED_BYTE, data);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, window_width_, window_height_, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
...
uint32_t OpenGLRenderReadPixel::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
