#include "opengl_render_texture.h"
...
void OpenGLRenderTexture::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTexture::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTexture::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderTexture::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderTexture::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
