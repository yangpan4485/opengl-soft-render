#include "opengl_render_texture_sub.h"
...
OpenGLRenderTextureSub::OpenGLRenderTextureSub() = default;

OpenGLRenderTextureSub::~OpenGLRenderTextureSub() = default;

bool OpenGLRenderTextureSub::Init() {
...
void OpenGLRenderTextureSub::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTextureSub::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTextureSub::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderTextureSub::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderTextureSub::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, width * 2, height * 2, 0, format, GL_UNSIGNED_BYTE, NULL);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, format, GL_UNSIGNED_BYTE, data);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, height, width, height, format, GL_UNSIGNED_BYTE, data);
    glTexSubImage2D(GL_TEXTURE_2D, 0, width, 0, width, height, format, GL_UNSIGNED_BYTE, data);
    glTexSubImage2D(GL_TEXTURE_2D, 0, width, height, width, height, format, GL_UNSIGNED_BYTE, data);
