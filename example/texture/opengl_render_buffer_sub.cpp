#include "opengl_render_buffer_sub.h"
...
namespace {
std::vector<float> point_vec{-1.0f, -1.0f, 0.0f,
                             -1.0f,  1.0f, 0.0f,
                              1.0f, -1.0f, 0.0f,
                              1.0f,  1.0f, 0.0f};

std::vector<float> texture_coord_vec{0.0f, 1.0f,
                                     0.0f, 0.0f,
                                     1.0f, 1.0f,
                                     1.0f, 0.0f};
}

OpenGLRenderBufferSub::OpenGLRenderBufferSub() = default;

OpenGLRenderBufferSub::~OpenGLRenderBufferSub() = default;

bool OpenGLRenderBufferSub::Init() {
    ...
    glBufferData(GL_ARRAY_BUFFER, (point_vec.size() +  texture_coord_vec.size()) * sizeof(float), NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0, point_vec.size() * sizeof(float), &point_vec[0]);
    glBufferSubData(GL_ARRAY_BUFFER, point_vec.size() * sizeof(float), texture_coord_vec.size() * sizeof(float), &texture_coord_vec[0]);

    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)((point_vec.size()) * sizeof(float)));
...
void OpenGLRenderBufferSub::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderBufferSub::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderBufferSub::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderBufferSub::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderBufferSub::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
