#include "opengl_render_texture_wrap_edge.h"
...
OpenGLRenderTextureWrapEdge::OpenGLRenderTextureWrapEdge() = default;

OpenGLRenderTextureWrapEdge::~OpenGLRenderTextureWrapEdge() = default;

bool OpenGLRenderTextureWrapEdge::Init() {
...
void OpenGLRenderTextureWrapEdge::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderTextureWrapEdge::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture.vs";
    return path;
}

std::string OpenGLRenderTextureWrapEdge::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;

}

bool OpenGLRenderTextureWrapEdge::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderTextureWrapEdge::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
