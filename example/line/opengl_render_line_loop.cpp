#include "opengl_render_line_loop.h"
...
OpenGLRenderLineLoop::OpenGLRenderLineLoop() {
...
bool OpenGLRenderLineLoop::Init() {
...
void OpenGLRenderLineLoop::Destroy() {
    glDeleteBuffers(1, &vbo_);
}

std::string OpenGLRenderLineLoop::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader.vs";
    return path;
}

std::string OpenGLRenderLineLoop::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader.fs";
    return path;
}

bool OpenGLRenderLineLoop::DrawGraphics(uint32_t width, uint32_t height) {
