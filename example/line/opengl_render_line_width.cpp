#include "opengl_render_line_width.h"
...
OpenGLRenderLineWidth::OpenGLRenderLineWidth() {
...
bool OpenGLRenderLineWidth::Init() {
...
void OpenGLRenderLineWidth::Destroy() {
    glDeleteBuffers(1, &vbo_);
}

std::string OpenGLRenderLineWidth::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader.vs";
    return path;
}

std::string OpenGLRenderLineWidth::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader.fs";
    return path;
}

bool OpenGLRenderLineWidth::DrawGraphics(uint32_t width, uint32_t height) {
