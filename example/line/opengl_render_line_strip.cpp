#include "opengl_render_line_strip.h"
...
OpenGLRenderLineStrip::OpenGLRenderLineStrip() {
...
bool OpenGLRenderLineStrip::Init() {
...
void OpenGLRenderLineStrip::Destroy() {
    glDeleteBuffers(1, &vbo_);
}

std::string OpenGLRenderLineStrip::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader.vs";
    return path;
}

std::string OpenGLRenderLineStrip::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader.fs";
    return path;
}

bool OpenGLRenderLineStrip::DrawGraphics(uint32_t width, uint32_t height) {
