#include "opengl_render_depth_test.h"
...
OpenGLRenderDepthTest::OpenGLRenderDepthTest() {
}

OpenGLRenderDepthTest::~OpenGLRenderDepthTest() {
}

bool OpenGLRenderDepthTest::Init() {
...
void OpenGLRenderDepthTest::Destroy() {
    glDeleteBuffers(1, &vbo_);
    glDeleteTextures(1, &texture_);
}

std::string OpenGLRenderDepthTest::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader_texture_advanced.vs";
    return path;
}

std::string OpenGLRenderDepthTest::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader_texture.fs";
    return path;
}

bool OpenGLRenderDepthTest::DrawGraphics(uint32_t width, uint32_t height) {
...
uint32_t OpenGLRenderDepthTest::LoadTexture(const std::string& texture_path) {
    uint32_t texture;
