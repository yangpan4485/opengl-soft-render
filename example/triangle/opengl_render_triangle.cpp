#include "opengl_render_triangle.h"
...
namespace {
    std::vector<float> vertices{ 0.0f,  0.5f, 0.0f, 0.3, 0.5, 0.7,
                                -0.5f, -0.5f, 0.0f, 0.3, 0.5, 0.7, 
                                 0.5f, -0.5f, 0.0f, 0.3, 0.5, 0.7,};
}

OpenGLRenderTriangle::OpenGLRenderTriangle() = default;

OpenGLRenderTriangle::~OpenGLRenderTriangle() = default;

bool OpenGLRenderTriangle::Init() {
...
void OpenGLRenderTriangle::Destroy() {
    glDeleteBuffers(1, &vbo_);
}

std::string OpenGLRenderTriangle::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader.vs";
    return path;
}

std::string OpenGLRenderTriangle::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader.fs";
    return path;
}

bool OpenGLRenderTriangle::DrawGraphics(uint32_t width, uint32_t height) {
