...
OpenGLRenderPoint::OpenGLRenderPoint() {
}

OpenGLRenderPoint::~OpenGLRenderPoint() {
}

bool OpenGLRenderPoint::Init() {
...
void OpenGLRenderPoint::Destroy() {
    glDeleteBuffers(1, &vbo_);
}

std::string OpenGLRenderPoint::GetVertexShader() {
    std::string path = process_path_ + "/shader_source/vertex_shader.vs";
    return path;
}

std::string OpenGLRenderPoint::GetFragmentShader() {
    std::string path = process_path_ + "/shader_source/fragment_shader.fs";
    return path;
}

bool OpenGLRenderPoint::DrawGraphics(uint32_t width, uint32_t height) {
